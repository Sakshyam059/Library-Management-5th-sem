...
@section('content')
    <div class="px-6 bg-white ">
        <div class="font-bold text-4xl py-6">
            <h1 class="inline-block">Books</h1>
            <div class="inline-flex float-right my-4">
                <a href="{{ route('admin.book.create') }}" class="bg-blue-600 text-white px-3 py-2 text-lg rounded">Add
                    Book</a>
            </div>
        ...
        <table class="display" id="mytable" class="mx-auto">
            <thead class="bg-stone-600 text-stone-50 text-xs lg:text-base">
                <th>S.N</th>
                <th>Title</th>
                <th>Author</th>
                <th class="hidden lg:table-cell">Edition</th>
            ...
            <tbody>
                @foreach ($books as $data)
                    <tr>
                        ...
                        <td>{{ $data->stock }}</td>
                        <td>
                            <img src="{{ asset('images/book/' . $data->image) }}" class="h-16" alt="">
                        </td>
                        <td>
                            <a href="{{ route('admin.book.edit', $data->id) }}"
                            ...
                            @section('deleteroute')
                            <form action="{{ route('book.delete', $data->id) }}" method="POST" class="inline-block mr-2">
                                @method('DELETE')
                                @csrf
